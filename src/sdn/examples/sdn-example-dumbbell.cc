...
main (int argc, char *argv[])
{
  ...
  const std::string animFile = "dumbbell.xml";
...
//
  ...
  anim.SetConstantPosition (controllerNode.Get(0), 65, 0);
  for (uint32_t i = 0; i < switchNodes.GetN(); ++i)
    {
      anim.SetConstantPosition (switchNodes.Get(i), 32.5 + 65.0*i/(switchNodes.GetN()-1), 65.0);
    }
  for (uint32_t i = 0; i < leftNodes.GetN(); ++i)
    {
      anim.SetConstantPosition (leftNodes.Get(i), 16, 32.5 + 65.0*i/(leftNodes.GetN()-1));
    }
  for (uint32_t i = 0; i < rightNodes.GetN(); ++i)
    {
      anim.SetConstantPosition (rightNodes.Get(i), 114, 32.5 + 65.0*i/(rightNodes.GetN()-1));
    }
